...
"""The result of this function calls is that the will return a value or
they will return an error. In order to fit this into the monadic error
handling framework. We need to convert that in functions that result
containers containing a success or failure track"""

"""And there is a decorator for that call safe"""
...
def verify_access(blog) -> 'Blog':
    ...
    if not blog_public:
        raise NotAuthorizedError(f'Your are not allowed to access blog with id {blog_id}')
    return blog
